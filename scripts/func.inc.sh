...
{ 
	...
	echo -e "Concatenating Fastqs From 10X together..."
        ...
        do
                cmd="gzip -cd ${fastq_dir}${i} >> ${output_dir}/${sample_name}.R1.fastq.gz"
                exec_cmd ${cmd} >> ${log} 2>&1
        done

        for i in $(ls ${fastq_dir} | grep R2 | grep fastq.gz)
        do
                cmd="gzip -cd ${fastq_dir}${i} >> ${output_dir}/${sample_name}.R2.fastq.gz"
                exec_cmd ${cmd} >> ${log} 2>&1
        done
 
        for i in $(ls ${fastq_dir} | grep R3 | grep fastq.gz)
        do
                cmd="gzip -cd ${fastq_dir}${i} >> ${output_dir}/${sample_name}.R3.fastq.gz"
                exec_cmd ${cmd} >> ${log} 2>&1
        done
